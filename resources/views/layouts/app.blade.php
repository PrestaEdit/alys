...
<body class="h-full bg-slate-50 font-sans antialiased">
    ...
    @php
        $hideBottomNav = request()->routeIs('onboarding');
        $needsBottomPad = !$hideBottomNav && !\Native\Mobile\Facades\System::isMobile();
    @endphp

    <main class="min-h-full {{ $needsBottomPad ? 'pb-20' : '' }}" style="padding-top: var(--safe-top);">
        {{ $slot }}
    </main>

    @unless($hideBottomNav)
        @mobile
            <native:bottom-nav active-color="#0ea5e9">
                <native:bottom-nav-item
                    id="home"
                    icon="{{ \Native\Mobile\Facades\System::isIos() ? 'house.fill' : 'home' }}"
                    label="{{ __('nav.home') }}"
                    url="{{ route('home') }}"
                ...
                <native:bottom-nav-item
                    id="calendar"
                    icon="{{ \Native\Mobile\Facades\System::isIos() ? 'calendar' : 'calendar_today' }}"
                    label="{{ __('nav.calendar') }}"
                    url="{{ route('calendar') }}"
                    :active="request()->routeIs('calendar')"
                />
                <native:bottom-nav-item
                    id="treatments"
                    icon="{{ \Native\Mobile\Facades\System::isIos() ? 'list.bullet' : 'list' }}"
                    label="{{ __('nav.treatments') }}"
                    url="{{ route('treatments') }}"
                    :active="request()->routeIs('treatments*')"
                />
                <native:bottom-nav-item
                    id="settings"
                    icon="{{ \Native\Mobile\Facades\System::isIos() ? 'gearshape.fill' : 'settings' }}"
                ...
                <div class="flex justify-around items-center h-16 max-w-lg mx-auto px-4">
                    ...
                    <a href="{{ route('calendar') }}"
                       class="flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-colors
                        ...
                        <span class="text-xs font-medium">{{ __('nav.calendar') }}</span>
                    </a>
                    ...
                    <a href="{{ route('treatments') }}"
                       class="flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-colors
                        ...
                        <span class="text-xs font-medium">{{ __('nav.treatments') }}</span>
                    </a>
                    ...
                    <a href="{{ route('settings') }}"
                       class="flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-colors
                        ...
                        <span class="text-xs font-medium">{{ __('nav.settings') }}</span>
                    </a>
